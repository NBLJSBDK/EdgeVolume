...
KDE Plasma Wayland 原生 KWin 输入过滤插件：鼠标位于虚拟屏幕最左侧 1px 时，滚轮调节 KDE 音量；点击、移动和拖拽不经过 KDEVolume 处理。
...
注销并重新登录 Wayland 会话后，检查左边缘滚轮行为。KDEVolume 作为 KWin 插件运行，不会出现单独的 kdevolume 进程。
...
- KDEVolume 只维护 Wayland 的 KWin 输入过滤路径；X11 会话不作为支持目标。
